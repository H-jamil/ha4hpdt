...
int main(int argc, char* argv[]) {
    if (argc != 5) { //       1              2                  3                   4               5              6         7       8
        printf("Usage: %s <hdf5_file><#compressionthreads><memory_node><compression_node> \n", argv[0]);
        return 1;
    }
    ...
    DataGenerator *generator = data_generator_init(data, *data_size, *y_dim * *x_dim * sizeof(uint16_t),0);
    if (generator == NULL) {
